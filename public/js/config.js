 ...
 */
UrlUniversalFile = 'https://tickets.cxsummit.com.co/CompraTickets/';
var TipoBoletas = 0;
var ArrayBoletas = [];
var UserId = 0;
var IdComprador = 0;
...
function nextPasoValEmpresa(val){
    ...
    if(val == 3){
        ...
        if(  t == 0 ){
            ...
            if( $('input[name=taller]:checked').val() == 3 || $('input[name=taller]:checked').val() == 5 ){
                $.ajax({
                    type:'POST',
                    ...
                    success:function(data){
                            if( data.info == 1 ){
                                location.href='https://tickets.cxsummit.com.co/CompraTickets/public/formPayUP/'+data.Id;
                            }
                    }
                })
            }else{
                ...
                $(".Contador").each(function(){
                            ...
                            $.ajax({
                                ...
                                success:function(data){
                                        if( data.info == 1 ){
                                            location.href='https://tickets.cxsummit.com.co/CompraTickets/public/ConfirmacionCompra';
                                        }
                                }
                            })
                        }
...
function MostrarBoletasGeneral(id){
	TipoBoletas = id;
	$(".TipoTik").hide("slow")
	$(".TipoTik"+id).show("slow")
	$(".BoletasGeneral").show("slow")
}

 function LoginPass(val){
	if( val == 1 ){
		var t = 0;
        var y = $("#Correo").val();
        if( y.length > 0 ){
	...
	if( val == 2 ){
		$.ajax({
			type:'POST',
			url:'DatosFacturacion',
			data:{
				codigo:UserId
				,_token:document.getElementsByName('_token')[0].value},
			success:function(data){
				if( TipoBoletas == 1 ){
					$("#F_namex").val( data.info[0]['razonsocial'] );
					$("#F_nit").val( data.info[0]['nit'] );
					validaasociado()
					$("#F_direccionx").val( data.info[0]['direccion'] );
					$("#F_persona").val( data.info[0]['personacontacto'] );
					$("#F_cargo").val( data.info[0]['cargo'] );
					$("#F_celularx").val( data.info[0]['celular'] );
					$("#F_mailx").val( data.info[0]['email'] );
					$("#F_acteco").val( data.info[0]['codigoacteco'] );
				}else if( TipoBoletas == 2 ){
					$("#F_name").val( data.info[0]['nombre'] );
					$("#F_cedula").val( data.info[0]['cedula'] );
					$("#F_direccion").val( data.info[0]['direccion'] );
					$("#F_telefono").val( data.info[0]['telefono'] );
					$("#F_celular").val( data.info[0]['celular'] );
					$("#F_mail").val( data.info[0]['email'] );
				}
				
			}
		})
		$(".FormasPago").hide("slow");
		$(".F"+TipoBoletas).show("slow");
		if( TipoBoletas == 1 ){
			$(".Contador").each(function(){
				if( $(this).text() != 0 ){
					ArrayBoletas.push({
                ...
                $(".Contador").each(function(){
                            ...
                            $.ajax({
                                ...
                                data:{
                                    ...
                                    F_nit:$("#F_nit").val(),
                                    F_Asociado:$("#F_Asociado").val(),
                                    F_direccion:$("#F_direccionx").val(),
                                    F_ciudad:$("#F_ciudadx").val(),
                                    F_pais:$("#F_paisx").val(),
                                    F_persona:$("#F_persona").val(),
                                    F_cargo:$("#F_cargo").val(),
                                    F_mail:$("#F_mailx").val(),
                                    F_celular:$("#F_celularx").val(),
                                    F_acteco:$("#F_acteco").val(),
                                    F_codigo:$("#F_codigo").val(),
                                    valorboleta:$(".valorboleta_").text(),
                                    impuesto:$(".impuesto_").text(),
...
function nextPasoVal(val){
    ArrayBoletas = [];
    if(val == 0){
        location.href='https://tickets.cxsummit.com.co/CompraTickets/public/SeleccionBoletas';
    }
    if( val === 1 ){
        var t = 0;
        var y = $("#Correo").val();
		...
		if( $('input[name=tallerxx1]:checked').val() == 1 ){
        }else{
...
function descargarpase(id){
    window.open('https://tickets.cxsummit.com.co/CompraTickets/public/Boleta/'+id);
}
